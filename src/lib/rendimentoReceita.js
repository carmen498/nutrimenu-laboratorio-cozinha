// Motor canônico de rendimento técnico — Fase 5.
//
// Conceitos separados:
 ...
 * Marcadores de sub-receita explodida são ignorados para não duplicar o peso;
 * quando não existem filhos explodidos, o marcador funciona como fallback.
 */
export function calcularPesoPrePreparo(receita, itens = []) {
  ...
  (itens || []).forEach((item) => {
    if (item?.subreceita_parent_id) parentsComFilhos.add(item.subreceita_parent_id);
  });
