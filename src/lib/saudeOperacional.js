...
export function calcularSaudeOperacional({ pagamentos = [], webhooks = [], emails = [] }) {
  ...
  const emailsSuprimidos = emails7d.filter((log) => log.status === "suprimido");
  ...
  return [
    ...
    { titulo: "Suprimidos", valor: emailsSuprimidos.length, detalhe: `${emailsSuprimidos.length} e-mails não enviados por produto_compra ausente nos últimos 7 dias`, status: emailsSuprimidos.length ? "alerta" : "ok" },
