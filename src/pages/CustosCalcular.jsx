...
export default function CustosCalcular() {
  ...
  const salvar = async () => {
    ...
    if (!resultado.valido) return toast.error("Configure a quantidade mensal de receitas antes de finalizar este cálculo.");
    ...
    <div className="space-y-5 pb-24 md:pb-8 max-w-6xl mx-auto">
      ...
      <div className="grid xl:grid-cols-[1fr_300px] gap-5 items-start">
        ...
        <div className="space-y-4 xl:sticky xl:top-20">
          ...
          <Card className="p-4 flex gap-3"><Info className="w-5 h-5 text-primary shrink-0" /><div><p className="text-sm font-medium">Rateio configurável, sem dupla contagem</p><p className="text-xs text-muted-foreground mt-1">Esta produção usa os grupos escolhidos em Configurações de Rateio. “Despesas com pessoal” permanece separado e a mão de obra direta desta receita é calculada pelas horas informadas aqui.</p><Link to="/custos/configuracoes" className="text-xs font-medium text-primary underline mt-2 inline-block">Revisar configurações de rateio</Link></div></Card>
          {!resultado.rateio.valido && <Card className="p-4 border-amber-300 bg-amber-50"><p className="text-sm font-medium text-amber-900">Rateio ainda não pode ser calculado</p><p className="text-xs text-amber-800 mt-1">Há despesas incluídas no rateio, mas a quantidade mensal de receitas está zerada. A formação do preço e a geração da ficha ficam bloqueadas para evitar custo subestimado.</p><Link to="/custos/configuracoes" className="text-xs font-medium text-amber-900 underline mt-2 inline-block">Configurar quantidade mensal</Link></Card>}
        </div>
      </div>
