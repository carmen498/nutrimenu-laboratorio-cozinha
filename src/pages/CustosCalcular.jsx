        ...
        <div className="space-y-4">
          ...
          <Card className="p-5 space-y-4">
            ...
            {loadingContexto ? <div className="flex items-center gap-2 text-sm text-muted-foreground"><Loader2 className="w-4 h-4 animate-spin" /> Calculando custo técnico...</div> : !receita ? <p className="text-sm text-muted-foreground">Selecione uma receita para calcular.</p> : <>
              ...
              <div className="rounded-lg bg-primary/5 border border-primary/20 p-4 grid sm:grid-cols-3 gap-3"><div><p className="text-xs text-muted-foreground">Custo total</p><p className="text-xl font-bold text-primary">{money(resultado.custoTotal)}</p></div><div><p className="text-xs text-muted-foreground">Custo por lote</p><p className="text-xl font-bold">{money(resultado.custoUnitario)}</p></div><div><p className="text-xs text-muted-foreground">Custo por porção</p><p className="text-xl font-bold">{totalPorcoes > 0 ? money(resultado.custoPorPorcao) : "—"}</p></div></div>
            </>}
          </Card>
