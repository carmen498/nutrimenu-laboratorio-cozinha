    ...
    <div className="space-y-5 pb-24 md:pb-8 max-w-6xl mx-auto">
      ...
      <div className="grid xl:grid-cols-[1fr_300px] gap-5 items-start">
        <div className="space-y-4">
          ...
          <Card className="p-5 space-y-4">
            ...
            {loadingContexto ? <div className="flex items-center gap-2 text-sm text-muted-foreground"><Loader2 className="w-4 h-4 animate-spin" /> Calculando custo técnico...</div> : !receita ? <p className="text-sm text-muted-foreground">Selecione uma receita para calcular.</p> : <>
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between gap-3"><span className="inline-flex items-center gap-2">Insumos e embalagens importados da receita {insumosImportados.length > 0 && <Popover><PopoverTrigger asChild><button type="button" className="text-xs text-primary underline">Ver itens</button></PopoverTrigger><PopoverContent className="w-[440px] text-sm" align="start"><p className="font-medium">Insumos e embalagens importados</p><p className="text-xs text-muted-foreground mt-1">Você pode desconsiderar um item somente neste cálculo ou removê-lo da receita de origem.</p><div className="mt-2 divide-y max-h-72 overflow-y-auto">{insumosImportados.map((item, index) => <div key={item.id || index} className="py-2.5"><div className="flex justify-between gap-3"><div><p>{item.insumo_nome || `Item ${index + 1}`}</p>{item.semPreco && <p className="text-xs text-amber-700">Sem preço cadastrado</p>}</div><strong className="whitespace-nowrap">{item.semPreco ? "—" : money(item.custoEscalado)}</strong></div><div className="flex flex-wrap gap-2 mt-2"><Button type="button" size="sm" variant="outline" className="h-7 text-xs" onClick={() => retirarDoCalculo("insumos", item.id)}>Não considerar neste cálculo</Button>{podeAlterarReceitaOrigem && <Button type="button" size="sm" variant="ghost" className="h-7 text-xs text-destructive hover:text-destructive" onClick={() => removerDaReceita("InsumoReceita", "insumos", item.id)}><Trash2 className="w-3 h-3 mr-1" /> Remover da receita</Button>}</div></div>)}</div></PopoverContent></Popover>}</span><strong>{money(tecnico?.custoInsumosTecnicos)}</strong></div>
                {Number(tecnico?.custoEsquecidos || 0) > 0 && <div className="flex justify-between gap-3"><span className="inline-flex items-center gap-2">Ingredientes esquecidos {esquecidosImportados.length > 0 && <Popover><PopoverTrigger asChild><button type="button" className="text-xs text-primary underline">Ver itens</button></PopoverTrigger><PopoverContent className="w-[440px] text-sm" align="start"><p className="font-medium">Ingredientes esquecidos / acabamento</p><p className="text-xs text-muted-foreground mt-1">Revise itens de processo e acabamento que entram no custo técnico.</p><div className="mt-2 divide-y max-h-72 overflow-y-auto">{esquecidosImportados.map((item, index) => <div key={item.id || index} className="py-2.5"><div className="flex justify-between gap-3"><div><p>{item.nome || `Item ${index + 1}`}</p><p className="text-xs text-muted-foreground">{Number(item.quantidade_g || 0).toLocaleString("pt-BR")} g na receita-base</p></div><strong className="whitespace-nowrap">{money(item.custoEscalado)}</strong></div><div className="flex flex-wrap gap-2 mt-2"><Button type="button" size="sm" variant="outline" className="h-7 text-xs" onClick={() => retirarDoCalculo("esquecidos", item.id)}>Não considerar neste cálculo</Button>{podeAlterarReceitaOrigem && <Button type="button" size="sm" variant="ghost" className="h-7 text-xs text-destructive hover:text-destructive" onClick={() => removerDaReceita("IngredienteEsquecidoReceita", "esquecidos", item.id)}><Trash2 className="w-3 h-3 mr-1" /> Remover da receita</Button>}</div></div>)}</div></PopoverContent></Popover>}</span><strong>{money(tecnico?.custoEsquecidos)}</strong></div>}
              </div>
          ...
          <div className="flex justify-end"><Button onClick={salvar} disabled={salvando || !receita || !tecnico?.completo || qtd <= 0 || !recalculoValido || !resultado.valido || !precoValido}>{salvando ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <FileText className="w-4 h-4 mr-2" />} {recalcularId ? "Salvar nova versão da ficha" : "Salvar cálculo e gerar ficha"}</Button></div>
        </div>

        <div className="space-y-4 xl:sticky xl:top-20">
          <Card className="p-5"><div className="flex items-center justify-between gap-3"><h2 className="font-semibold">Resumo</h2>{receita && !recalcularId && <span className="rounded-full bg-amber-100 px-2 py-1 text-[10px] font-semibold text-amber-800">RASCUNHO</span>}</div><div className="space-y-3 mt-4 text-sm"><div className="flex justify-between"><span className="text-muted-foreground">Receita</span><strong className="text-right max-w-[160px] truncate">{receita?.nome || "—"}</strong></div><div className="flex justify-between"><span className="text-muted-foreground">Produção</span><strong>{receita && qtd > 0 ? `${qtd} receita(s)` : "—"}</strong></div><div className="flex justify-between"><span className="text-muted-foreground">Custo técnico</span><strong>{receita ? money(tecnico?.custoTecnicoTotal) : "—"}</strong></div>{totalInsumosAdicionais > 0 && <div className="flex justify-between"><span className="text-muted-foreground">Insumos adicionais</span><strong>{money(totalInsumosAdicionais)}</strong></div>}<div className="flex justify-between"><span className="text-muted-foreground">Custo do Negócio</span><strong>{receita ? money(resultado.rateio.custoDaProducao) : "—"}</strong></div><div className="border-t pt-3 flex justify-between"><span className="font-medium">Custo total da produção</span><strong className="text-primary">{receita ? money(resultado.custoTotal) : "—"}</strong></div><div className="flex justify-between"><span className="text-muted-foreground">Custo por receita</span><strong>{receita ? money(resultado.custoUnitario) : "—"}</strong></div>{precoValido && <><div className="border-t pt-3 flex justify-between"><span className="font-medium">Preço de venda por receita</span><strong>{money(precoVendaEfetivo)}</strong></div><div className="flex justify-between"><span className="text-muted-foreground">Margem</span><strong>{Number(formacaoDireta.margem || 0).toFixed(1).replace(".", ",")}%</strong></div><div className="flex justify-between"><span className="text-muted-foreground">Markup</span><strong>{Number(formacaoDireta.markup || 0).toFixed(2).replace(".", ",")}x</strong></div></>}<div className="border-t pt-4"><Button className="w-full" onClick={salvar} disabled={salvando || !receita || !tecnico?.completo || qtd <= 0 || !recalculoValido || !resultado.valido || !precoValido}>{salvando ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <FileText className="w-4 h-4 mr-2" />}{recalcularId ? "Salvar nova versão da ficha" : "Salvar cálculo e gerar ficha"}</Button><p className="text-[11px] text-muted-foreground text-center mt-2">{receita ? "Só depois de salvar este cálculo aparecerá no Histórico." : "Selecione uma receita para iniciar."}</p></div></div></Card>
