...
export default function CustosConfiguracoes() {
  ...
  const gruposPersistidos = Array.isArray(config?.grupos_rateio_incluidos) && config.grupos_rateio_incluidos.length > 0
  ...
  const salvar = async () => {
    ...
    try {
      const payload = {
        user_id: user.id,
        metodo_rateio_padrao: "unidade_produzida",
        volume_mensal_estimado: volumeN,
        unidade_volume: "unidades_mes",
      ...
      <div className="grid lg:grid-cols-[1fr_320px] gap-5 items-start">
        <div className="space-y-4">
          <Card className="p-5 space-y-4">
            <div className="flex gap-3"><div className="w-10 h-10 rounded-xl bg-primary/10 text-primary flex items-center justify-center"><Calculator className="w-5 h-5" /></div><div><h2 className="font-semibold text-lg">Base do rateio</h2><p className="text-xs text-muted-foreground mt-1">No MVP, as despesas são divididas pelo volume mensal estimado em lotes/unidades de produção.</p></div></div>
            <div className="grid sm:grid-cols-2 gap-4">
              <div><label className="text-sm font-medium">Volume mensal estimado</label><Input className="mt-1" type="number" min="0" step="1" value={volumeEfetivo} onChange={(e) => setVolume(e.target.value)} placeholder="Ex.: 200" /><p className="text-[11px] text-muted-foreground mt-1">unidades de produção por mês</p></div>
              <div><label className="text-sm font-medium">Método de rateio</label><div className="mt-1 h-9 rounded-md border bg-muted/40 px-3 flex items-center text-sm">Por unidade produzida</div><p className="text-[11px] text-muted-foreground mt-1">Único método disponível no MVP.</p></div>
            </div>
          </Card>
        ...
        <div className="space-y-4 lg:sticky lg:top-4">
          <Card className="p-5">
            ...
            <div className="space-y-3 mt-4 text-sm"><div className="flex justify-between"><span className="text-muted-foreground">Despesas incluídas</span><strong>{money(totalIncluido)}</strong></div><div className="flex justify-between"><span className="text-muted-foreground">Volume mensal</span><strong>{num(volumeEfetivo) > 0 ? `${num(volumeEfetivo)} un.` : "—"}</strong></div><div className="border-t pt-3"><p className="text-xs text-muted-foreground">Custo rateado por unidade</p><p className="text-3xl font-bold text-primary mt-1">{num(volumeEfetivo) > 0 ? money(custoRateado) : "—"}</p></div></div>
            {num(volumeEfetivo) <= 0 && totalIncluido > 0 && <div className="mt-4 rounded-lg border border-amber-300 bg-amber-50 p-3 text-xs text-amber-900 flex gap-2"><AlertCircle className="w-4 h-4 shrink-0" /><span>Informe o volume mensal para calcular o rateio por unidade.</span></div>}
            {num(volumeEfetivo) > 0 && <div className="mt-4 rounded-lg border border-primary/20 bg-primary/5 p-3 text-xs text-primary flex gap-2"><CheckCircle2 className="w-4 h-4 shrink-0" /><span>Resumo consistente: {money(totalIncluido)} ÷ {num(volumeEfetivo)} = {money(custoRateado)} por unidade.</span></div>}
          </Card>
