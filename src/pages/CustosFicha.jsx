    ...
    <div className="space-y-5 pb-24 md:pb-8 max-w-6xl mx-auto">
      ...
      <div className="grid lg:grid-cols-[1.25fr_.75fr] gap-4 items-start">
        ...
        <Card className="p-5">
          ...
          <div className="space-y-3 mt-4 text-sm"><div className="flex justify-between"><span className="text-muted-foreground">{formacaoAssistida ? "Preço sugerido aplicado" : "Preço informado"}</span><strong>{preco > 0 ? money(preco) : "—"}</strong></div><div className="flex justify-between"><span className="text-muted-foreground">Custo por receita</span><strong>{money(calculo.custo_unitario)}</strong></div>{formacaoAssistida && custosComercializacaoUnitario > 0 && <div className="flex justify-between"><span className="text-muted-foreground">Custos de comercialização</span><strong>{money(custosComercializacaoUnitario)}</strong></div>}{formacaoAssistida && taxasVariaveisPct > 0 && <div className="flex justify-between text-xs"><span className="text-muted-foreground">Cartão/plataforma + impostos</span><strong>{pct(taxasVariaveisPct)}</strong></div>}{formacaoAssistida && custoFixoVenda > 0 && <div className="flex justify-between text-xs"><span className="text-muted-foreground">Custo fixo por receita vendida</span><strong>{money(custoFixoVenda)}</strong></div>}<div className="flex justify-between"><span className="text-muted-foreground">Lucro {formacaoAssistida ? "líquido" : "estimado"} por receita</span><strong>{preco > 0 ? money(lucroUnitario) : "—"}</strong></div><div className="flex justify-between"><span className="text-muted-foreground">{formacaoAssistida ? "Margem líquida" : "Margem estimada"}</span><strong>{preco > 0 ? pct(calculo.margem_estimada) : "—"}</strong></div></div>
