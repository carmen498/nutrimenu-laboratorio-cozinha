...
import { AlertCircle, ArrowLeft, Calculator, ChevronRight, ExternalLink, FileText, Pencil, RefreshCw, WalletCards } from "lucide-react";
...
export default function CustosFicha() {
  ...
  const lucroUnitario = preco - Number(calculo?.custo_unitario || 0) - custoFixoVenda - taxasVendaUnitario;
    ...
    <div className="space-y-5 pb-24 md:pb-8 max-w-6xl mx-auto">
      <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" onClick={() => navigate(`/custos/calcular?receita=${encodeURIComponent(calculo.origem_id)}`)}><RefreshCw className="w-4 h-4 mr-2" /> Recalcular com preços atuais</Button>
          <Button disabled><Pencil className="w-4 h-4 mr-2" /> Editar preço de venda · próxima fase</Button>
        </div>
      </div>
      ...
      <Card className="p-4 sm:p-5">
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          ...
          <div className="flex-1 min-w-0"><h2 className="font-display text-xl font-bold truncate">{calculo.origem_nome_snapshot}</h2><div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground mt-2"><span>{categoria}</span><span>Origem: Laboratório de Cozinha</span><span>{numero(calculo.quantidade_produzida)} {calculo.unidade_producao || "lote(s)"}</span><span>Calculado em {dataHora(calculo.data_calculo)}</span></div></div>
      ...
      <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-3">
        ...
        <Card className="p-4"><p className="text-xs text-muted-foreground">Custo unitário</p><p className="text-2xl font-bold mt-1">{money(calculo.custo_unitario)}</p><p className="text-[11px] text-muted-foreground mt-1">por {calculo.unidade_producao === "lotes" ? "lote" : "unidade"}</p></Card>
        <Card className="p-4"><p className="text-xs text-muted-foreground">Preço de venda</p><p className="text-2xl font-bold mt-1">{preco > 0 ? money(preco) : "—"}</p><p className="text-[11px] text-muted-foreground mt-1">valor informado</p></Card>
        <Card className="p-4"><p className="text-xs text-muted-foreground">{formacaoAssistida ? "Margem líquida" : "Margem estimada"}</p><p className="text-2xl font-bold mt-1">{preco > 0 ? pct(calculo.margem_estimada) : "—"}</p><p className="text-[11px] text-muted-foreground mt-1">{formacaoAssistida ? "após taxas informadas" : "sobre o preço de venda"}</p></Card>
      </div>
      ...
      <div className="grid lg:grid-cols-[1.25fr_.75fr] gap-4 items-start">
        ...
        <Card className="p-5">
          ...
          <div className="space-y-3 mt-4 text-sm"><div className="flex justify-between"><span className="text-muted-foreground">Preço informado</span><strong>{preco > 0 ? money(preco) : "—"}</strong></div><div className="flex justify-between"><span className="text-muted-foreground">Custo por unidade</span><strong>{money(calculo.custo_unitario)}</strong></div>{formacaoAssistida && taxasVariaveisPct > 0 && <div className="flex justify-between"><span className="text-muted-foreground">Taxas da venda</span><strong>{money(taxasVendaUnitario)}</strong></div>}{formacaoAssistida && custoFixoVenda > 0 && <div className="flex justify-between"><span className="text-muted-foreground">Custo fixo de venda</span><strong>{money(custoFixoVenda)}</strong></div>}<div className="flex justify-between"><span className="text-muted-foreground">Lucro {formacaoAssistida ? "líquido" : "estimado"} por unidade</span><strong>{preco > 0 ? money(lucroUnitario) : "—"}</strong></div><div className="flex justify-between"><span className="text-muted-foreground">{formacaoAssistida ? "Margem líquida" : "Margem"}</span><strong>{preco > 0 ? pct(calculo.margem_estimada) : "—"}</strong></div></div>
          {preco > 0 ? <div className={`mt-4 rounded-lg p-3 text-sm ${margemPositiva ? "bg-primary/5 border border-primary/20 text-primary" : "bg-red-50 border border-red-200 text-red-800"}`}>{margemPositiva ? "Este preço cobre os custos e gera resultado positivo." : "Este preço está abaixo do custo calculado."}</div> : <div className="mt-4 rounded-lg bg-muted/50 p-3 text-sm text-muted-foreground">Nenhum preço de venda foi informado nesta ficha.</div>}
        </Card>
      </div>
      ...
      <Card className="overflow-hidden">
        <Accordion type="single" collapsible value={complementaresOpen ? "complementares" : ""} onValueChange={(v) => setComplementaresOpen(v === "complementares")}>
          <AccordionItem value="complementares" className="border-0"><AccordionTrigger className="px-5 py-4 hover:no-underline"><div className="text-left"><p className="font-semibold">Informações complementares</p><p className="text-xs font-normal text-muted-foreground mt-1">Parâmetros, valores técnicos e detalhes do cálculo.</p></div></AccordionTrigger><AccordionContent className="px-5 pb-5"><div className="grid md:grid-cols-2 gap-4 text-sm"><div className="rounded-lg border p-4 space-y-2"><h3 className="font-semibold">Parâmetros da produção</h3><div className="flex justify-between"><span className="text-muted-foreground">Quantidade produzida</span><strong>{numero(calculo.quantidade_produzida)} {calculo.unidade_producao}</strong></div><div className="flex justify-between"><span className="text-muted-foreground">Porções estimadas</span><strong>{numero(calculo.total_porcoes_snapshot, 1)}</strong></div><div className="flex justify-between"><span className="text-muted-foreground">Rendimento base</span><strong>{numero(calculo.rendimento_snapshot)} {calculo.rendimento_unidade_snapshot || ""}</strong></div><div className="flex justify-between"><span className="text-muted-foreground">Versão do motor técnico</span><strong>{calculo.modelo_tecnico_versao || "—"}</strong></div></div><div className="rounded-lg border p-4 space-y-2"><h3 className="font-semibold">Snapshot técnico</h3><div className="flex justify-between"><span className="text-muted-foreground">Ingredientes</span><strong>{money(calculo.custo_ingredientes_snapshot)}</strong></div><div className="flex justify-between"><span className="text-muted-foreground">Insumos técnicos</span><strong>{money(calculo.custo_insumos_tecnicos_snapshot)}</strong></div><div className="flex justify-between"><span className="text-muted-foreground">Ingredientes esquecidos</span><strong>{money(calculo.custo_esquecidos_snapshot)}</strong></div><div className="flex justify-between"><span className="text-muted-foreground">Custo técnico total</span><strong>{money(calculo.custo_tecnico_snapshot)}</strong></div></div><div className="rounded-lg border p-4 space-y-2"><h3 className="font-semibold">Formação do custo</h3><div className="flex justify-between"><span className="text-muted-foreground">Embalagem adicional</span><strong>{money(calculo.custo_embalagens)}</strong></div><div className="flex justify-between"><span className="text-muted-foreground">Mão de obra</span><strong>{money(calculo.custo_mao_obra)}</strong></div><div className="flex justify-between"><span className="text-muted-foreground">Rateio</span><strong>{money(calculo.custo_rateado)}</strong></div><div className="flex justify-between"><span className="text-muted-foreground">Custo por porção</span><strong>{money(calculo.custo_por_porcao)}</strong></div></div><div className="rounded-lg border p-4 space-y-2"><h3 className="font-semibold">Preço</h3><div className="flex justify-between"><span className="text-muted-foreground">Método</span><strong>{formacaoAssistida ? "Margem assistida" : "Preço informado"}</strong></div><div className="flex justify-between"><span className="text-muted-foreground">Markup aplicado</span><strong>{Number(calculo.markup_aplicado || 0) > 0 ? `${numero(calculo.markup_aplicado)}x` : "—"}</strong></div><div className="flex justify-between"><span className="text-muted-foreground">{formacaoAssistida ? "Margem líquida" : "Margem estimada"}</span><strong>{preco > 0 ? pct(calculo.margem_estimada) : "—"}</strong></div>{formacaoAssistida && <><div className="flex justify-between"><span className="text-muted-foreground">Taxa cartão/plataforma</span><strong>{pct(calculo.taxa_cartao_pct)}</strong></div><div className="flex justify-between"><span className="text-muted-foreground">Impostos</span><strong>{pct(calculo.impostos_pct)}</strong></div>{custoFixoVenda > 0 && <div className="flex justify-between"><span className="text-muted-foreground">Custo fixo por unidade</span><strong>{money(custoFixoVenda)}</strong></div>}</>}<div className="flex justify-between"><span className="text-muted-foreground">Status</span><strong>{calculo.status}</strong></div><div className="flex justify-between"><span className="text-muted-foreground">Versão da ficha</span><strong>{calculo.versao_calculo || 1}</strong></div></div></div><div className="mt-4 flex flex-wrap gap-2"><Button variant="outline" disabled><FileText className="w-4 h-4 mr-2" /> Exportar PDF · próxima fase</Button><Button variant="outline" disabled>Imprimir · próxima fase</Button><Button variant="outline" disabled>Compartilhar · próxima fase</Button></div></AccordionContent></AccordionItem>
        </Accordion>
      </Card>
