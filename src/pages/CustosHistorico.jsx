            ...
            <div className="md:hidden divide-y">
              {exibidos.map((c) => (
                <div key={c.id} className="p-4 space-y-3">
                  ...
                  {c.formacao_preco_metodo === "margem" && <div className="flex flex-wrap gap-1.5"><Badge variant="outline" className="text-[10px]">Formação avançada</Badge></div>}
              ...
              <tbody className="divide-y">
                {exibidos.map((c) => (
                  <tr key={c.id} className="group hover:bg-muted/25">
                    <td className="sticky left-0 z-10 bg-background group-hover:bg-muted/25 px-4 py-3 align-top"><div className="min-w-0"><Link to={`/custos/ficha/${c.id}`} className="font-medium hover:text-primary line-clamp-2 leading-snug" title={c.origem_nome_snapshot}>{c.origem_nome_snapshot}</Link>{c.formacao_preco_metodo === "margem" && <div className="flex flex-wrap gap-1.5 mt-2"><Badge variant="outline" className="text-[10px]">Formação avançada</Badge></div>}</div></td>
