  ...
  try {
    ...
    for (const receita of receitasSlice) {
      ...
      for (const item of itens) {
        ...
        // Calculate cost
        const ingInfo = ingMap[item.ingrediente_id];
        if (ingInfo && ingInfo.preco_por_g_rs != null && novoTotalG !== null) {
          custoTotalReceita += novoTotalG * ingInfo.preco_por_g_rs;
        }
      }

      // Update receita if anything changed
      if (teveAlteracao || rendimentoTotal > 0) {
        const custoPorPorcao = porcoesBase > 0 ? custoTotalReceita / porcoesBase : 0;
        await base44.asServiceRole.entities.Receita.update(receita.id, {
          rendimento_total: Math.round(rendimentoTotal * 100) / 100,
          custo_total: Math.round(custoTotalReceita * 100) / 100,
          custo_por_porcao: Math.round(custoPorPorcao * 100) / 100,
        });
        log.push(`📊 [${receita.nome}] rendimento: ${rendimentoTotal.toFixed(1)}g | custo total: R$${custoTotalReceita.toFixed(2)} | custo/porção: R$${custoPorPorcao.toFixed(2)}`);
      }
    }
