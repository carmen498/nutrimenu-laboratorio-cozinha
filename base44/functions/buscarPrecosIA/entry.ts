...
Deno.serve(async (req) => {
  try {
    ...
    const ingNames = ingredientes.map((ing, i) => `${i + 1}. ${ing.nome}`).join('\n');
...
- Retorne um array na mesma ordem dos ingredientes informados`;
    ...
    const resultados = (result.resultados || []).slice(0, ingredientes.length);
