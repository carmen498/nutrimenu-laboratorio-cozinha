...
function normalizarCiclo(valor: unknown): number {
  const ciclo = Number(valor || 0);
  return Number.isFinite(ciclo) && ciclo > 0 ? Math.floor(ciclo) : 0;
}

function diferencaDiasISO(dataAlvo: string, dataBase: string): number {
...
export function avaliarElegibilidadeRenovacao(user: any, agora = new Date()) {
  ...
  if (ciclo < 1) {
    return { elegivel: false, motivo: "ciclo_insuficiente", ciclo };
  }
...
export function proximoCicloRenovacao(plano: string, cicloAtual: unknown): number {
  const ciclo = normalizarCiclo(cicloAtual);
  if (plano === "anual") return Math.max(ciclo, 1);
  if (plano === "renovacao") return Math.max(ciclo, 1) + 1;
  return ciclo;
}

export function cicloRenovacaoAposEstorno(plano: string, cicloAtual: unknown): number {
  const ciclo = normalizarCiclo(cicloAtual);
  if (plano === "anual" || plano === "renovacao") return Math.max(0, ciclo - 1);
  return ciclo;
}
