...
const n = (v: any) => num(v).toFixed(10);
...
export function serializarDeterminantesCusto({
  ...
  const linhasInsumos = (insumos || []).map((item: any) => [
    'N',
    txt(item.insumo_id),
  ...
  const linhasEsquecidos = (esquecidos || []).map((item: any) => {
    const ingrediente = item.ingrediente_id ? resolverIngrediente(txt(item.ingrediente_id)) : null;
    const preco = ingrediente ? resolverPreco(ingrediente) : 0;
...
export function gerarAssinaturaCusto(args: any = {}) {
